...
function scripts() {
  return src("src/scripts/*.js").pipe(dest("dev/scripts"));
}
...
function images() {
  return (
    src(["src/img/**/*.{jpg,png}", "!src/img/**/*-lazy.{jpg,png}"])
...
function imageCopy() {
  return src("src/img/**/*.{jpg,png,gif}").pipe(dest("dev/img"));
}
...
function cssProduction() {
  return (
    src("dev/styles/*.css")
      ...
      .pipe(
        purgecss({
          content: ["dev/**/*.html", "dev/scripts/*.js"],
        ...
        cleanCss(
          ...
          (details) => {
            ...
            console.log(
              details.name +
                " is now " +
                details.stats.minifiedSize / 100 +
                " bytes"
            );
          }
        )
...
function scriptsProduction() {
  return src("dev/scripts/*.js").pipe(terser()).pipe(dest("public/scripts"));
}

// Image processing – copies over images
function imgProduction() {
  return src("dev/img/**/*.{jpg,gif,png}").pipe(dest("public/img/"));
}

// Font processing – copies over fonts
function typefaceProduction() {
  return src("dev/styles/*.woff").pipe(dest("public/styles/"));
}
